...
#c17 <- unique(d17[c("col","desc","chave")])
#write.csv2(c17, file = "/Users/felipedaroz/Google Drive/Academia/Doutorado/Dados_municipios/c17.csv")
...
d17_1 <- d17_1 %>%
  left_join(unique(m_17[c("municipio","idhm","faixa_idh")]), by = "municipio") 
...
mun$code_muni <- as.integer(mun$code_muni)
d17_1$cod_ibge <- as.integer(d17_1$cod_ibge)
...
m3 <- ggplot2::ggplot(mun2) +
                    ...
                    theme(panel.grid.major = element_line(colour="gray85", size=0.5),
                          legend.title=element_blank()) +
  ggtitle("Receita própria com Transferências Redistributivas, per capita", subtitle = paste0("(",length(unique(d17_1$municipio))," municípios")) 
