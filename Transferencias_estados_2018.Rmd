...
# upload de dados RECEITA 2018 direto do zip
  # estados
d <- read.csv2(unzip("/Users/felipedaroz/Google Drive/Academia/Doutorado/Dados_estados/finbra_ESTDF_ReceitasOrcamentarias(AnexoI-C)_2018.zip", "finbra.csv"), skip = 3, header = FALSE)
  # municipios
...
# criando tabela com cada municipio, codigo ibge e populacao
#mun <- unique(d[c("col","desc","cod_tipo","chave")])
#write.csv2(mun, file = "/Users/felipedaroz/Google Drive/Academia/Doutorado/Dados_municipios/cod.csv")
...
est_14 <- left_join(dic_rend, d, by = "chave") 

# reorganizando df
est_14 <- est_14[,c(6,7,8,9,10,11,13,2,3,4,5,12)]

# Preparando dados
est_14$categoria <- as.factor(est_14$categoria)
est_14$valor <- as.numeric(sub(",", ".",est_14$valor, fixed = TRUE ))
#est_14$populacao <- as.numeric(levels(est_14$populacao)[est_14$populacao])
est_14$populacao <- as.numeric(est_14$populacao)
...
Receita própria - categorias 6, 7, 8, 10, 11
Transferências
...
est_14$tipo <- as.character(est_14$tipo)
tipo_rec <- est_14 %>%
  filter(tipo != "") %>%
  group_by(nome) %>%
